...
SUBROUTINE MELT_RH87orig(DIN, TLAYER, PLAYER, RLAYER, VT, SEKDEL, DENSE, DENSA, DOUT)
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !!!  This is a spherical hail melting estimate based on Rasmussen
  !!!  and Heymsfield 1987 Table 1 eqs. 4-5.  
  !!!
...
!f2py REAL*8, INTENT(IN) :: TLAYER, PLAYER, RLAYER, VT
      ...
      !ESATW=611.*EXP(ALV_SI/rv*(RATIO-1./TLAYER))
      !RHOOMGW=ESATW/(rv*TLAYER)
      !ESATI=611.*EXP(ALS_SI/rv*(RATIO-1./TLAYER))
      !RHOOMGI=ESATI/(rv*TLAYER)
      ...
      !! print out all the terms and see which goes nan when TC>0
    !   print *, 'DCM', DCM
    !   print *, 'AH', AH
    !   print *, 'AK', AK
    !   print *, 'tclayer', tclayer
    !   print *, 'AE', AE
    !   print *, 'ALV', ALV
    !   print *, 'DI', DI
    !   print *, 'DELRWC', DELRWC
    ...
    !   print *, 'RHOKOR', RHOKOR
    !   print *, 'RHOAIR', RHOAIR
    !   print *, 'eenv_si', eenv_si
    !   print *, 'RE', RE
    !   print *, 'Term 1:', (2.*PI*DCM*AH*AK*tclayer)/ALF
    !   print *, 'TERM 2:', ( 2.*PI*DCM*AE*ALV*DI*DELRWC)/ALF
    !   print *, 'final dmlt in kg:', dmlt
    !   print *, ' '
  ...
  END SUBROUTINE MELT_RH87orig

  SUBROUTINE MELT_RH87update(DIN, TLAYER, PLAYER, RLAYER, VT, SEKDEL, DENSE, DENSA, DOUT)
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    !!!  This is a spherical hail melting estimate based on Rasmussen
    !!!  and Heymsfield 1987 Table 1 eqs. 1-5, instead of just 4-5.  
    !!!
    !!!  INPUT:  DIN      diameter prior to melting 
    !!!          SEKDEL   timstep (s)
    !!!          TLAYER   environmental temperature (K)
  ...
  !f2py REAL*8, INTENT(IN) :: TLAYER, PLAYER, RLAYER, VT
        ...
        !add check for small Reynolds numbers, to apply eq. (1) from RH87 Table 1.
        if ( RE .LT. 250 ) then
          dmlt = -SEKDEL  / ALF * (2.*PI*DCM*AH*AK*tclayer + 2.*PI*DCM*AE*ALV*DI*DELRWC) * 2.0 ! gram        
        else
          dmlt = -SEKDEL  / ALF * (2.*PI*DCM*AH*AK*tclayer + 2.*PI*DCM*AE*ALV*DI*DELRWC) ! gram
        endif
        dmlt = dmlt/1000 ! convert g to kg
    ...
    END SUBROUTINE MELT_RH87update
